// Description:

// Write two functions that convert a roman numeral to and from an integer value. Multiple roman numeral values will be tested for each function.

// Modern Roman numerals are written by expressing each digit separately starting with the left most digit and skipping any digit with a value of zero. In Roman numerals 1990 is rendered: 1000=M, 900=CM, 90=XC; resulting in MCMXC. 2008 is written as 2000=MM, 8=VIII; or MMVIII. 1666 uses each Roman symbol in descending order: MDCLXVI.

// Input range : 1 <= n < 4000

// In this kata 4 should be represented as IV, NOT as IIII (the "watchmaker's four").
// Examples

// to roman:
// 2000 -> "MM"
// 1666 -> "MDCLXVI"
// 1000 -> "M"
//  400 -> "CD"
//   90 -> "XC"
//   40 -> "XL"
//    1 -> "I"

// from roman:
// "MM"      -> 2000
// "MDCLXVI" -> 1666
// "M"       -> 1000
// "CD"      -> 400
// "XC"      -> 90
// "XL"      -> 40
// "I"       -> 1

// Help
// Symbol 	Value
// I 		1
// IV 		4
// V 		5
// X 		10
// L 		50
// C 		100
// D 		500
// M 		1000

class RomanNumerals {
  static toRoman(num) {
    const romanNumerals = {
      M: 1000,
      CM: 900,
      D: 500,
      CD: 400,
      C: 100,
      XC: 90,
      L: 50,
      XL: 40,
      X: 10,
      IX: 9,
      V: 5,
      IV: 4,
      I: 1,
    };

    let result = '';

    for (const symbol in romanNumerals) {
      while (num >= romanNumerals[symbol]) {
        result += symbol;
        num -= romanNumerals[symbol];
      }
    }

    return result;
  }

  static fromRoman(str) {
    const romanNumerals = {
      M: 1000,
      D: 500,
      C: 100,
      L: 50,
      X: 10,
      V: 5,
      I: 1,
    };

    let result = 0;

    for (let i = 0; i < str.length; i++) {
      const currentSymbol = romanNumerals[str[i]];
      const nextSymbol = romanNumerals[str[i + 1]];

      if (nextSymbol > currentSymbol) {
        result += nextSymbol - currentSymbol;
        i++;
      } else {
        result += currentSymbol;
      }
    }

    return result;
  }
}

console.log(toRoman(2000)); // "MM"
console.log(toRoman(1666)); // "MDCLXVI"
console.log(toRoman(1000)); // "M"
console.log(toRoman(400)); // "CD"
console.log(toRoman(90)); // "XC"
console.log(toRoman(40)); // "XL"
console.log(toRoman(1)); // "I"

console.log(fromRoman('MM')); // 2000
console.log(fromRoman('MDCLXVI')); // 1666
console.log(fromRoman('M')); // 1000
console.log(fromRoman('CD')); // 400
console.log(fromRoman('XC')); // 90
console.log(fromRoman('XL')); // 40
console.log(fromRoman('I')); // 1
