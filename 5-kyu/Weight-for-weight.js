...
function orderWeight(strng) {
   ...
   function customSort(a, b) {
     ...
     function weightOfNumber(num) {
       return num.split("").reduce((sum, digit) => sum + parseInt(digit), 0);
     }
 
     // Compare the weights
     const weightA = weightOfNumber(a);
     const weightB = weightOfNumber(b);
 
     if (weightA === weightB) {
       // If weights are equal, compare as strings
       return a.localeCompare(b);
     } else {
       // Otherwise, compare based on weights
       return weightA - weightB;
     }
   }
 
   // Sort the weights using the custom sort function
   const sortedWeights = weights.sort(customSort);
 
   // Join the sorted weights back into a string
   const result = sortedWeights.join(" ");
